...
resource "google_storage_bucket_iam_binding" "bigquery_classify_storage_bucket_iam" { 
  bucket = var.bigquery_classify_data
...
resource "google_cloudfunctions2_function" "bigquery_classify_function" {
  name     = "bigquery-classify-function"
  project  = var.gcp_project
  location = var.gcp_region
  ...
  build_config {
    ...
    entry_point = "run"
    environment_variables = {
      GOOGLE_FUNCTION_SOURCE = "bigquery_classify.py"
    }

    source {
      storage_source {
        bucket = var.bigquery_classify_bucket
        object = var.bigquery_classify_object
      }
    }
  }
  ...
  service_config {
    ...
    environment_variables = {
      GOOGLE_FUNCTION_SOURCE = "bigquery_classify.py"
      GCS_BUCKET_ID = var.bigquery_classify_data
    }
  }
}
...
output "bigquery_classify_function_uri" {
  value = google_cloudfunctions2_function.bigquery_classify_function.service_config[0].uri
}
